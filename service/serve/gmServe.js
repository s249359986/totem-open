...
*/
function composeGif(options) {
  return new Promise(function (resolve, reject) {
    let tempObj = getFormatStr(options)
    console.log('getFilePath', tempObj)
    let tempFileName = getFileName()
    let tempFilePath = dir + '/' + tempFileName
    let downLoadPath = config['openDomain'] + '/' + config['downloadDirPath'] + '/' + tempFileName
    gm().in(...tempObj).write(tempFilePath, err => {
      if (err) {
        reject(err)
      } else {
        resolve({ code: 0, data: { path: downLoadPath } })
      }
    })
  })
}
module.exports = composeGif